    ...
    def create_network(self, net_info, input_width = None, input_height = None, input_channels = None):
        ...
        while i < layer_num:
            ...
            if ltype == 'Convolution':
                ...
                if convolution_param.has_key('kernel_size'):
                    kernel_size = int(convolution_param['kernel_size'])
                elif convolution_param.has_key('kernel_h') & convolution_param.has_key('kernel_w'):
                    kernel_h = int(convolution_param['kernel_h'])
                    kernel_w = int(convolution_param['kernel_w'])
                    kernel_size = (kernel_h, kernel_w)

                if convolution_param.has_key('stride'):
                    stride = int(convolution_param['stride'])
                elif convolution_param.has_key('stride_h') & convolution_param.has_key('stride_w'):
                    stride_h = int(convolution_param['stride_h'])
                    stride_w = int(convolution_param['stride_w'])
                    stride = (stride_h, stride_w)
                else:
                    stride = 1
                if convolution_param.has_key('pad'):
                    pad = int(convolution_param['pad'])
                elif convolution_param.has_key('pad_h') & convolution_param.has_key('pad_w'):
                ...
                if type(kernel_size) is tuple:
                    blob_width[tname] = (blob_width[bname] + 2 * pad[1] - kernel_size[1]) / stride[1] + 1
                    blob_height[tname] = (blob_height[bname] + 2 * pad[0] - kernel_size[0]) / stride[0] + 1
                else:
                ...
                if layer['convolution_param'].has_key('kernel_size'):
                    kernel_size = int(layer['convolution_param']['kernel_size'])
                elif layer['convolution_param'].has_key('kernel_h') & layer['convolution_param'].has_key('kernel_w'):
                ...
                elif layer['convolution_param'].has_key('stride_h') & layer['convolution_param'].has_key('stride_w'):
                ...
                if layer['convolution_param'].has_key('pad'):
                    padding = int(layer['convolution_param']['pad'])
                elif layer['convolution_param'].has_key('pad_h') & layer['convolution_param'].has_key('pad_w'):
